...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    state.background = ImLerp(state.background, popup_open ? ImColor(20, 20, 20) : hovered ? ImColor(25, 25, 25) : ImColor(32, 33, 35), g.IO.DeltaTime * 6.0f);
    state.text = ImLerp(state.text, popup_open ? ImColor(255, 255, 255, 255) : hovered ? ImColor(180, 180, 180, 180) : ImColor(107, 107, 107, 107), g.IO.DeltaTime * 6.0f);
    ...
    else
    {
        ...
        if (flags & ImGuiComboFlags_HeightSmall)  popup_max_height_in_items = 4;
        else if (flags & ImGuiComboFlags_HeightLarge)  popup_max_height_in_items = 20;
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * state.combo_size));
    }
